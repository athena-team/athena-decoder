...
template <class W> class WeightFromString {
 public:
  W operator()(const std::string &s);
};

// Generic case fails.
template <class W> inline
W WeightFromString<W>::operator()(const std::string &s) {
  //FSTERROR() << "VectorFst::Read: Obsolete file format";
    std::cerr << "VectorFst::Read: Obsolete file format";
  return W::NoWeight();
}

// TropicalWeight version.
template <> inline
TropicalWeight WeightFromString<TropicalWeight>::operator()(const std::string &s) {
  float f;
...
// LogWeight version.
template <> inline
LogWeight WeightFromString<LogWeight>::operator()(const std::string &s) {
  float f;
  memcpy(&f, s.data(), sizeof(f));
  return LogWeight(f);
}
